...
export default function Treasury(){
  ...
  return <div className="mx-auto min-h-screen max-w-5xl px-6 pb-24 pt-36">
    ...
    <div className="mt-5 border border-[#E9E1CF22] p-7"><p className="mono text-[10px] text-[#777269]">CORE AUTHORITY</p><p className="mt-3 text-sm">Status: {String(org?.status??"—")} / steward: <span className="break-all">{String(org?.current_steward??"—")}</span> / spending: {String(org?.spending_enabled??"—")}</p><p className="mono mt-3 break-all text-[9px] text-[#777269]">Core→Vault {binding?.coreVault??"—"}<br/>Vault→Core {binding?.vaultCore??"—"}</p><p className="mono mt-2 text-[9px] text-[#B8784E]">Dormant recovery eligible: {String(canRecover)}</p><VaultActions orgId={id} onChanged={reread} canDeposit={org?.status==="ACTIVE"&&org?.sealed===true} canRelease={org?.status==="ACTIVE"&&org?.spending_enabled===true} canRecover={canRecover}/><p className="mono mt-4 break-words text-[10px] text-[#B8784E]">{state}</p></div>
  </div>;
}
