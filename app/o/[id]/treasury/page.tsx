...
export default function Treasury(){
  ...
  const [org,setOrg]=useState<Record<string,any>>();const [vault,setVault]=useState<Record<string,any>>();const [allowance,setAllowance]=useState<bigint>();const [canRecover,setCanRecover]=useState(false);const [binding,setBinding]=useState<{coreVault:string,vaultCore:string}>();const [state,setState]=useState("READING CORE / VAULT");
  ...
  return <div className="mx-auto min-h-screen max-w-5xl px-6 pb-24 pt-36">
    ...
    <div className="mt-12 grid gap-4 md:grid-cols-5">{metrics.map(([key,value])=><div key={String(key)} className="border border-[#E9E1CF22] p-5"><p className="mono text-[10px] text-[#777269]">{String(key)}</p><p className="serif mt-4 break-words text-2xl">{value===undefined?"—":`${formatGen(value as string|number|bigint)} GEN`}</p><p className="mono mt-2 break-all text-[9px] text-[#777269]">{value===undefined?"":`${String(value)} wei`}</p></div>)}</div>
    <div className="mt-5 border border-[#E9E1CF22] p-7"><p className="mono text-[10px] text-[#777269]">CORE AUTHORITY</p><p className="mt-3 text-sm">Status: {String(org?.status??"—")} / steward: <span className="break-all">{String(org?.current_steward??"—")}</span> / spending: {String(org?.spending_enabled??"—")}</p><p className="mono mt-3 break-all text-[9px] text-[#777269]">Core→Vault {binding?.coreVault??"—"}<br/>Vault→Core {binding?.vaultCore??"—"}</p><p className="mono mt-2 text-[9px] text-[#B8784E]">Dormant recovery eligible: {String(canRecover)}</p><VaultActions orgId={id} onChanged={reread}/><p className="mono mt-4 break-words text-[10px] text-[#B8784E]">{state}</p></div>
  </div>;
}
