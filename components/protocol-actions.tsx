...
export function ProtocolActions({orgId}:{orgId:string}){
  ...
  const write=async(functionName:string,args:CalldataEncodable[],verify:()=>Promise<void>)=>{
    ...
    try{
      ...
      const final=await confirmWrite(session.client,hash,verify);updateStoredTransaction(actionKey,session.address,final.stage);setState(final.stage==="STATE_CONFIRMED"?final.stage:`${final.stage}: ${final.error??"not confirmed"}`);
...
export function VaultActions({orgId,onChanged,canDeposit=true,canRelease=true}:{orgId:string,onChanged?:()=>Promise<void>,canDeposit?:boolean,canRelease?:boolean}){
  const {address,ensureWriteReady}=useWallet();
  ...
  const finish=async(client:ReturnType<typeof createClient>,hash:`0x${string}`,verify:()=>Promise<void>,actionKey:string,method:string,args:unknown[],account:string,transfer?:{recipient:string,value:bigint})=>{rememberSubmittedTransaction({actionKey,account,chainId:"0xf22f",contract:env.vaultAddress,method,args,hash});setTx(hash);setState(`SUBMITTED ${hash}`);const final=await confirmWrite(client,hash,async()=>{await verify();await onChanged?.()});if(final.stage==="STATE_CONFIRMED"&&transfer){try{setChild(await findTriggeredTransfer(client,hash,transfer.recipient,transfer.value))}catch{setState("STATE_CONFIRMED / child transfer not yet observed")}}updateStoredTransaction(actionKey,account,final.stage);setState(final.stage==="STATE_CONFIRMED"?final.stage:`${final.stage}: ${final.error??"not confirmed"}`)};
