...
export function useWallet(){
 const [address,setAddress]=useState<string>();const [chainOk,setChainOk]=useState(false);const [error,setError]=useState<string>();
 const sync=useCallback(async()=>{if(!window.ethereum)return;const accounts=await window.ethereum.request({method:"eth_accounts"}) as string[];const chain=await window.ethereum.request({method:"eth_chainId"}) as string;setAddress(accounts[0]);setChainOk(Number(chain)===env.chainId)},[]);
 ...
 return {address,chainOk,error,connect,disconnect:()=>{setAddress(undefined);setChainOk(false)},switchNetwork,readClient:createClient({chain:studionet}),writeClient:address?(createClient({chain:studionet,account:address as `0x${string}`,provider:window.ethereum}) as any):undefined};
}
