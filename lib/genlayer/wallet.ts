...
export async function ensureStudionet(current: EIP1193Provider) {
  ...
  if (chainId.toLowerCase() !== STUDIONET_CHAIN_ID) {
    try { await current.request({ method: "wallet_switchEthereumChain", params: [{ chainId: STUDIONET_CHAIN_ID }] }); }
      ...
      try {
        await current.request({ method: "wallet_addEthereumChain", params: [{ chainId: STUDIONET_CHAIN_ID, chainName: "GenLayer Studionet", nativeCurrency: { name: "GEN", symbol: "GEN", decimals: 18 }, rpcUrls: [env.rpc], blockExplorerUrls: [env.explorer] }] });
...
type WalletContext = {
  ...
  ensureWriteReady: () => Promise<{ client: ReturnType<typeof createClient>; provider: EIP1193Provider; address: string }>;
...
export function WalletProvider({ children }: { children: React.ReactNode }) {
  ...
  const sync = useCallback(async () => {
    ...
    if (!current) { setAddress(undefined); setChainId(undefined); setChainOk(false); return; }
    try {
      const state = await readProviderState(current);
  ...
  const ensureWriteReady = async () => {
    const current = getProvider(); if (!current || !address) throw new Error("Connect a wallet before signing.");
    const state = await readProviderState(current); if (!state.address || state.address.toLowerCase() !== address.toLowerCase()) throw new Error("Wallet account changed. Reconnect before signing.");
    if (!state.chainOk) { await ensureStudionet(current); await sync(); }
    const verified = await readProviderState(current); if (!verified.chainOk) throw new Error("WRONG_NETWORK: switch wallet to Studionet 61999");
